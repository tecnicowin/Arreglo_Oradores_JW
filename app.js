...
const app = {
    ...
    // --- UI Rendering ---
    renderLists() {
        // Render Congregaciones
        const congList = document.getElementById('list-congregaciones');
        if (congList) {
            congList.innerHTML = '';
            for (const [id, name] of Object.entries(this.db.congregaciones)) {
                congList.innerHTML += `
        ...
        if (bosqList) {
            bosqList.innerHTML = '';
            for (const [id, title] of Object.entries(this.db.bosquejos)) {
                bosqList.innerHTML += `
                    <div class="glass-card" style="display: flex; justify-content: space-between; align-items: center; padding: 12px 16px;">
                        <div style="flex: 1;">
    ...
    updateArreglosFlow() {
        ...
        tipo.addEventListener('change', () => {
            if (tipo.value === 'Saliente') {
                cong.value = '3780 - San Bernardino';
                this.filterOradoresByCong('3780');
            } else {
                cong.value = '';
